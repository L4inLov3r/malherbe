...
6. **Hedging épistémique** (« ces résultats suggèrent », « dans la limite de cet échantillon ») : compétence académique, les copies les mieux notées modalisent. Test de départage, dans l'ordre : (1) la modalisation porte-t-elle sur une affirmation identifiable ? (2) l'incertitude est-elle motivée par le dossier (limite de méthode, d'échantillon, de données) ? (3) au plus un modalisateur par proposition ? Trois oui → légitime, ne pas toucher. Sinon → candidat S12, mais en académique ne corriger que l'empilement manifeste (3+ modalisateurs sur une phrase qui n'affirme rien) — dans le doute, s'abstenir : abîmer une nuance épistémique coûte plus cher devant un jury que laisser un modalisateur de trop.

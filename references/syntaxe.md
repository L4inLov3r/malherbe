...
Détection : 2+ triades par page dont les éléments ne sont pas repris ensuite (test de reprise). EXCLUSIONS MAJEURES : (1) ne comptent JAMAIS les énumérations de référents CONCRETS (objets, ingrédients, gestes, dates, détails sensoriels), même multiples — le tic est la triade d'ABSTRACTIONS calibrées (adjectifs d'éloge, bénéfices, concepts interchangeables), pas la liste de choses réelles d'une recette ou d'un récit ; (2) la rhétorique ternaire est culturellement valorisée en français (tradition classique) — une triade isolée, même abstraite, reste normale. Ne jamais corriger seul.
